...
async function main() {
  ...
  const updateStmt = db.prepare(`
    UPDATE images SET title = ?, hash = ? WHERE id = ?;
  `);
    ...
    try {
      ...
      if (row.title === '' && row.hash === '') {
        getImageCaption(filePath).then(output => { 
          ...
          updateStmt.run(output[0].generated_text, hash, row.id)
...
//     // getImageCaption(filePath).then(output => { 
//     //   // Generate image title and hash...
//     //   //console.log('generated_text = ', output[0].generated_text) 
//     //   const hash = sha256FileSync(filePath)

//     //   updateImageTitle.run(output[0].generated_text, hash, id)
//     // }).catch(error => {
//     //   console.log(error)
//     // })
